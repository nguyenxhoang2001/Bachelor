...
def build_model(problem):
    ...
    D = {}
    for i in problem.tasks:
        D[i] = model.addVar(vtype=GRB.CONTINUOUS, name = f"D_{i}")
    ...
    W = model.addVar(vtype=GRB.CONTINUOUS, name = f"W")
...
def add_constraints(model, problem, X, Y, D, Z, W):
    ...
    for k in problem.qcs:
        for j in problem.tasks:
            if (k,0,j) in X:
                model.addConstr(
                    problem.earliest_time[k] - D[j] + problem.starting_travel_time.get((k,j),0) + problem.duration[j] <= problem.M * (1 - X[(k,0,j)]),
                    name=f"earliest_start_{k}_{j}"
                )
